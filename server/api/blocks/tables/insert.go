...
func HandleInsert(metadataDB *gorm.DB) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if db.Table(fmt.Sprintf("%s.%s", *block.Name, *table.Name)).Where(query_str).Updates(data).RowsAffected == 0 {
			err := db.Table(fmt.Sprintf("%s.%s", *block.Name, *table.Name)).Create(&data).Error
